...
from user_profile.api.serializers import BitSerializer, ProfileSerializer
...
class ProfileViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if len(profile_list) == 1:
            serializer_class = ProfileSerializer(profile_list, many = False)
...
class BitViewSet(viewsets.ViewSet):
        ...
        if self.request.query_params:
            ...
            if self.request.query_params.get("dataset"):
                ...
                if dataset == "liked":
                    ...
                    bit_pool = these_interactions.liked_bits.all()
                if dataset == "disliked":
                    bit_pool = these_interactions.disliked_bits.all()
                if dataset == "commented":
                    bit_pool = these_interactions.commented_on.all()
                if dataset == "shared":
                    bit_pool = these_interactions.shared_bits.all()
                if dataset == "saved":
                    pass
                ...
                if type == "global":
                    my_bits = Bit.objects.filter(profile = user_profile)[start:end]
                    friends_bits = Bit.objects.prefetch_related('custom', 'user').filter(profile__in = user_profile.connections.all())[start:end]
                    follow_bits = Bit.objects.prefetch_related('custom', 'user').filter(profile__in = user_profile.connections.all())[start:end]
                    ...
                    my_bits = Bit.objects.filter(profile = user_profile, type=type)[start:end]
                    friends_bits = Bit.objects.prefetch_related('custom', 'user').filter(profile__in = user_profile.connections.all(), type = type)[start:end]
                    follow_bits = Bit.objects.prefetch_related('custom', 'user').filter(profile__in = user_profile.connections.all(), type=type)[start:end]
                    all_bits = Bit.objects.prefetch_related('custom', 'user').filter(is_public = True, type=type).order_by('-like_count')[start:end]
    ...
    def list(self, request):
        ...
        if len(bit_pool) == 0:
            ...
            serializer_class = BitSerializer(bit_pool, many=True)
            return Response({"bits_found":True, "content":serializer_class.data})
    ...
    def retrieve(self, request, pk=None):
        ...
        serializer_class = BitSerializer(self.bit, many=False)
